                    ...
                    <div class="col d-flex flex-column">
                        <div>
                            <div class="row align-items-center p-2 m-2">
                                <div class="col-auto">
                                    <h2>Modifier un project</h2>
                                </div>
                            </div>
                            <div class="card-body">
                                <form method="POST" action="{{ route('admin.tenants.projects.update' , ['tenant' => $tenant->id, 'project' => $project->id]) }}" id="ajaxForm" role="form">
                                    @csrf
                                    @method('PUT')
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <div class="mb-3">
                                                ...
                                                <input id="name" type="text" class="form-control @error('name') is-invalid @enderror" name="name"
                                                    value="{{ old('name') ?? $project->name }}" placeholder="project name"  autocomplete="name" autofocus>
                                                @error('name')
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select type="text" class="form-select" placeholder="Select project groups" id="select-groups" name="groups[]" value="" multiple>
                                                    @foreach($groups as $group)
                                                        <option value="{{ $group->id }}"  {{ in_array($group->id , $projectGroups) ? 'selected' : '' }}>{{ $group->name }}</option>
                                                    @endforeach
                                                </select>
                                                @error('visibility')
                                                    <span class="invalid-feedback" role="alert">
