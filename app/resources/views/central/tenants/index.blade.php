                ...
                @if (count($tenants) > 0)
                    <div class="col-auto ms-auto d-print-none">
                        <div class="btn-list">
                            ...
                            <a href="{{ route('admin.tenants.create') }}" class="btn btn-red d-sm-none btn-icon"
                                data-bs-toggle="tooltip" data-bs-placement="top" title="Créer un nouveau tenant">
                                <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24"
                                    viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none"
                                    stroke-linecap="round" stroke-linejoin="round">
                                    <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                                    ...
                                    <div id="table-tenants" class="table-responsive">
                                        ...
                                        <div class="table-responsive">
                                            <table class="table card-table table-vcenter text-nowrap datatable">
                                                <thead>
                                                    <tr>
                                                        <th><button class="table-sort" data-sort="sort-doceboidorg">Docebo
                                                                ORG_ID</button></th>
                                                        <th><button class="table-sort" data-sort="sort-zendeskidorg">ZENDESK
                                                                ORG_ID</button></th>
                                                        <th><button class="table-sort" data-sort="sort-code">Code</button>
                                                        </th>
                                                        <th><button class="table-sort" data-sort="sort-name">Nom</button>
                                                        </th>
                                                        <th><button class="table-sort"
                                                                data-sort="sort-domain">Domain</button></th>
                                                        <th><button class="table-sort" data-sort="sort-created">Date de
                                                                création</button></th>
                                                        <th><button class="table-sort" data-sort="sort-updated">Date de
                                                                modification</button></th>
                                                            ...
                                                            <tr>
                                                                ...
                                                                @production
                                                                    <td class="sort-domain">
                                                                        <a href="{{ 'https://' . $tenant->domains->first()->domain . '.' . config('app.domain') }}"
                                                                            target="_blank">
                                                                            <span
                                                                                class="badge badge-outline text-red">{{ 'https://' . $tenant->domains->first()->domain . '.' . config('app.domain') }}</span>
                                                                        </a>
                                                                    </td>
                                                                @else
                                                                    <td class="sort-domain">
                                                                        <a href="{{ 'http://' . $tenant->domains->first()->domain . '.' . config('app.domain') }}"
                                                                            target="_blank">
                                                                            <span
                                                                                class="badge badge-outline text-red">{{ 'http://' . $tenant->domains->first()->domain . '.' . config('app.domain') }}</span>
                                                                        </a>
                                                                    </td>
                                                                @endproduction

                                                                <td class="sort-created"
                                                                    data-created="{{ date('U', strtotime($tenant->created_at)) }}">
                                                                    {{ date('d-m-Y H:i:s', strtotime($tenant->created_at)) }}
                                                                </td>
                                                                <td class="sort-created"
                                                                    data-created="{{ date('U', strtotime($tenant->updated_at)) }}">
                                                                ...
                                                                <td class="text-end">
                                                                    ...
                                                                    <a href="{{ route('admin.tenants.destroy', $tenant->id) }}"
                                                                        ...
                                                                        onclick="event.preventDefault(); document.getElementById('{{ 'delete-form_' . $tenant->id }}').submit();">
                                                                        <svg xmlns="http://www.w3.org/2000/svg"
                                                    ...
                                                    @else
                                                        <tr>
                                                            <td colspan="6">
                                                                ...
                                                                <p class="empty-title text-danger text-center">Aucun
                                                                    résultat trouvé</p>
                                                            </td>
                                                        </tr>
                                                    @endif
