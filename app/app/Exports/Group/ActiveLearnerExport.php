...
class ActiveLearnerExport implements FromArray, WithMapping, WithHeadings, WithStrictNullComparison, WithTitle, ShouldAutoSize, WithStyles
{
    ...
    public function array(): array
    {
        if ($this->datedebut != null && $this->datefin != null) {
            $learners = Learner::where('statut', 'active')->whereBetween('last_access_date', [$this->datedebut, $this->datefin])->where('group_id', $this->groupId)->get()->toArray();
        } else {
            $learners = Learner::where('statut', 'active')->where('group_id', $this->groupId)->get()->toArray();
        }
        return $learners;
    }
